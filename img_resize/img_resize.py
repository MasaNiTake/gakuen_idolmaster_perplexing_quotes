...
def trim_and_save_avif(input_path, output_path, trim_values, target_size_kb=None):
  """画像をトリミングし、指定サイズ以下になるようにAVIF形式で保存する。
  ...
  try:
    ...
    # トリミング
    # ※ cropの座標は (left, top, right, bottom) で、rightとbottomは含まれない右端/下端の座標
...
trim_values = (3, 46, 3, 3)  # 例: 左3px, 上46px, 右3px, 下3px をトリミング
